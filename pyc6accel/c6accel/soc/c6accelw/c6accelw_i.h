...
/* In the multicore ARM+DSP devices the wrapper has to manage the shared memory.
  ...
  therefore there is no need to flush the cache. However, to keep it simple for ARM user and minimize the risk of unintended
  errors the cache management will always be enabled.
  This has an overhead so a future optimization will be to control this with a flag.
