...
def test(img, thres):
    ...
    t = cv.GetTickCount()
    pyc6accel.Threshold(img, out_pyc6accel, 20, thres)
    t = cv.GetTickCount() - t
    msec = t / (cv.GetTickFrequency() * 1000.)
    print "pyc6accel Threshold % s detection time = %g ms" % (thres,msec)
    cv.SaveImage('test_files/result_Threshold_pyc6accel_%s.png' % thres, out_pyc6accel)
#    fd.write(str(msec) + '\t')
    out_opencv = cv.CreateImage(cv.GetSize(img), img.depth, img.nChannels)
    t = cv.GetTickCount()
    cv.Threshold(img, out_opencv, 20, 255, cv.CV_THRESH_BINARY)
    t = cv.GetTickCount() - t
    msec = t / (cv.GetTickFrequency() * 1000.)
    print "opencv Threshold detection time = %g ms" % msec
    cv.SaveImage('test_files/result_Threshold_opencv.png', out_opencv)
##    fd.write(str(msec) + '\t')
    
if __name__ == '__main__' :
    ''''''
    for i in range(1):
        test(lenaimg,pyc6accel.CV_THRESH_BINARY)
