        ...
        String algName, UNIVERSAL_Handle hUni)
{
	C6accel_Handle hC6accel =
	        (C6accel_Handle) calloc(1, sizeof(C6accel_Object));
	...
	if (hUni == NULL) {
		if (algName == NULL) {
			...
			hC6accel->hUni = UNIVERSAL_create(hC6accel->hEngine, algName,
			        &uniParams);
...
)
{
	...
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_fft16x16_Params)
	...
	fp0 = (DSP_fft16x16_Params *) ((XDAS_Int8*) CInArgs
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_ifft16x16_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_ifft16x16_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_fft32x32_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_fft32x32_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_ifft32x32_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_ifft32x32_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_autocor_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_autocor_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
int * ptr_r, int nx) /* Number of elements */

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_dotprod_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Set function Id and parameter pointers for first function call */
	fp0 = (DSP_dotprod_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->x_InArrID1 = INBUF0;
...

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(gen_twiddle_fft16x16_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Set function Id and parameter pointers for first function call */
	fp0 = (gen_twiddle_fft16x16_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->n = n;
...

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(gen_twiddle_fft32x32_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Set function Id and parameter pointers for first function call */
	fp0 = (gen_twiddle_fft32x32_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->n = n;
	fp0->twiddle_OutArrID1 = OUTBUF0;
...
int C6accel_DSP_dotp_sqr(C6accel_Handle hC6accel, int Gin,
        ...
        short * restrict ptr_y, /*Input array. Must be double-word aligned */
        int * restrict ptr_r, int * restrict ptr_Gout, int nx) /* Number of elements */
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_dotp_sqr_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_dotp_sqr_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
 ...
 */
int C6accel_DSP_mat_mul(C6accel_Handle hC6accel, short *restrict ptr_x, /* Input matrix 1 */
...
int qs) /* Number of elements */
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_mat_mul_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_mat_mul_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        short * restrict ptr_h, short * restrict ptr_r, int nh, int nr)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_fir_gen_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_fir_gen_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        short * restrict ptr_h, short * restrict ptr_r, int nh, int nr)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_fir_r4_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_fir_r4_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	fp0->x_InArrID1 = INBUF0;
	fp0->h_InArrID2 = INBUF1;
	fp0->r_OutArrID1 = OUTBUF0;
        ...
        short * restrict ptr_h, short * restrict ptr_r, int nh, int nr)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_fir_r8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_fir_r8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        short * restrict ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSP_iir_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSP_iir_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_histogram_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_histogram_8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_histogram_16_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_histogram_16_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_median_3x3_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_median_3x3_8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_median_3x3_16_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_median_3x3_16_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_median_3x3_16s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_median_3x3_16s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_median_3x3_8_Frame_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_median_3x3_8_Frame_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...

{
	...
	InArg_Buf_size = sizeof(Fxn_struct)
	        + sizeof(IMG_median_3x3_16_Frame_Params) + sizeof(CInArgs->size)
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_median_3x3_16_Frame_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct)
	        + sizeof(IMG_median_3x3_16s_Frame_Params) + sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_median_3x3_16s_Frame_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int x_dim, const char *restrict mask, int shift)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_conv_3x3_i8_c8s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_conv_3x3_i8_c8s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        short width, short pitch, const short *restrict mask_ptr, short shift)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_conv_3x3_i16s_c16s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_conv_3x3_i16s_c16s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int rows, int x_dim, const char *restrict mask, int shift)
{
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_conv_3x3_i8_c8s_Frame_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        short shift)
{
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_conv_3x3_i16s_c16s_Frame_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        const char *restrict mask_ptr, short shift)
{
	...
	/* Define pointer to function parameter structure */
	IMG_conv_5x5_i8_c8s_Params * fp0;
	XDAS_Int8 *pAlloc;
	ACQUIRE_CODEC_ENGINE;

	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_conv_5x5_i8_c8s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	/* Request contiguous heap memory allocation for the extended input structure */
	pAlloc = (XDAS_Int8 *) Memory_alloc(InArg_Buf_size, &wrapperMemParams);
	CInArgs = (IC6Accel_InArgs *) pAlloc;
	...
	/* Fill in input/output buffer descriptor parameters and manage ARM cache*/
	/* See wrapper_c6accel_i.h for more details of operation                 */
	CACHE_WB_INV_INPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(imgin_ptr, 0, width * pitch * sizeof(unsigned char));
	CACHE_WB_INV_INPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(mask_ptr, 1, 5 * 5 *sizeof(char));
	CACHE_INV_OUTPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(imgout_ptr, 0, width * pitch * sizeof(unsigned char));
	...
	/* Set function Id and parameter pointers for first function call */
	CInArgs->fxn[0].FxnID = IMG_CONV_5X5_I8_C8S_FXN_ID;
	CInArgs->fxn[0].Param_ptr_offset = sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns) + MAX_FXN_CALLS * sizeof(Fxn_struct);


	/* Initialize pointers to function parameters */
	fp0 = (IMG_conv_5x5_i8_c8s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int x_dim, const unsigned char *restrict mask, short shift, int round)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_corr_3x3_i8_c8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_corr_3x3_i8_c8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        short pitch, const short *restrict mask_ptr, short shift, int round)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_corr_3x3_i16s_c16s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_corr_3x3_i16s_c16s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int round)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct)
	        + sizeof(IMG_corr_3x3_i8_c8_Frame_Params) + sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_corr_3x3_i8_c8_Frame_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int round)
{
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_corr_3x3_i16s_c16s_Frame_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
int C6accel_IMG_sobel_3x3_8(C6accel_Handle hC6accel,
        const IplImage *restrict in, /* Input image data   */
        IplImage *restrict out, /* Output image data  */
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_sobel_3x3_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Fill in input/output buffer descriptor parameters and manage ARM cache*/
	/* See wrapper_c6accel_i.h for more details of operation                 */
	CACHE_WB_INV_INPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(in->imageData,0,in->height * in->widthStep *sizeof(char));
	CACHE_INV_OUTPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(out->imageData,0,out->height * out->widthStep *sizeof(char));
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_sobel_3x3_8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_sobel_3x3_8_16_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_sobel_3x3_8_16_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_sobel_3x3_16_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_sobel_3x3_16_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_mulS_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_mulS_16s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_mulS_16s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_addS_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Fill in input/output buffer descriptor parameters and manage ARM cache*/
	/* See wrapper_c6accel_i.h for more details of operation                 */
	CACHE_WB_INV_INPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(imgR->imageData,0,imgR->height*imgR->widthStep * sizeof(char));
	CACHE_INV_OUTPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(imgW->imageData,0,imgW->height*imgW->widthStep * sizeof(char));
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_addS_8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_addS_16s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_addS_16s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_add_16s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Set function Id and parameter pointers for first function call */
	CInArgs->fxn[0].FxnID = IMG_ADD_16S_FXN_ID;
	CInArgs->fxn[0].Param_ptr_offset = sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns) + sizeof(Fxn_struct);


	/* Initialize pointers to function parameters */
	fp0 = (IMG_add_16s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);


	/* Fill in the fields in the parameter structure */
	fp0->imgR1_InArrID1 = INBUF0;
	fp0->imgR2_InArrID2 = INBUF1;
	fp0->imgW_OutArrID1 = OUTBUF0;
	fp0->count = count;
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_subS_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_subS_8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_subS_16s_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_subS_16s_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_yc_demux_le8_c_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_yc_demux_le8_c_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_yc_demux_be8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_yc_demux_be8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_yc_demux_le16_16_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_yc_demux_le16_16_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_yuv420pl_to_rgb565_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_yuv420pl_to_rgb565_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int num_pixels)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct)
	        + sizeof(IMG_ycbcr422pl_to_rgb565_Params) + sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_ycbcr422pl_to_rgb565_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int num_pixels)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct)
	        + sizeof(IMG_ycbcr422pl16_to_rgb565_Params) + sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_ycbcr422pl16_to_rgb565_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct)
	        + sizeof(IMG_yuv420pl16_to_rgb565_Params) + sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_yuv420pl16_to_rgb565_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_thr_gt2max_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	/* Request contiguous heap memory allocation for the extended input structure */
	pAlloc = (XDAS_Int8 *) Memory_alloc(InArg_Buf_size, &wrapperMemParams);
	CInArgs = (IC6Accel_InArgs *) pAlloc;
...
/* image threshold greater than to threshold 8 bits
 //Int C6accel_IMG_thr_gt2thr_8(C6accel_Handle hC6accel,const unsigned char * in_data, unsigned char * restrict out_data, short cols, short rows, unsigned char threshold
 //in_data : Input image data
 //out_data : Output image data
 //cols, rows, :Image dimensions
 //threshold : Threshold value
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_thr_gt2thr_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	/* Request contiguous heap memory allocation for the extended input structure */
	pAlloc = (XDAS_Int8 *) Memory_alloc(InArg_Buf_size, &wrapperMemParams);
	CInArgs = (IC6Accel_InArgs *) pAlloc;
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_thr_gt2thr_8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->indata_InArrID1 = INBUF0;
	fp0->outdata_OutArrID1 = OUTBUF0;
...
/* image threshold less than to minimum 8 bits
 //Int C6accel_IMG_thr_le2min_8(C6accel_Handle hC6accel,const unsigned char * in_data, unsigned char * restrict out_data, short cols, short rows, unsigned char threshold
 //in_data : Input image data
 //out_data : Output image data
 //cols, rows, :Image dimensions
 //threshold : Threshold value
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_thr_le2min_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	/* Request contiguous heap memory allocation for the extended input structure */
	pAlloc = (XDAS_Int8 *) Memory_alloc(InArg_Buf_size, &wrapperMemParams);
	CInArgs = (IC6Accel_InArgs *) pAlloc;
	...
	/* Set function Id and parameter pointers for first function call */
	CInArgs->fxn[0].FxnID = IMG_THR_LE2MIN_8_FXN_ID;
	CInArgs->fxn[0].Param_ptr_offset = sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns) + sizeof(Fxn_struct);


	/* Initialize pointers to function parameters */
	fp0 = (IMG_thr_le2min_8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->indata_InArrID1 = INBUF0;
	fp0->outdata_OutArrID1 = OUTBUF0;
...
/* image threshold less than to threshold 8 bits
 //Int C6accel_IMG_thr_le2thr_8(C6accel_Handle hC6accel,const unsigned char * in_data, unsigned char * restrict out_data, short cols, short rows, unsigned char threshold
 //in_data : Input image data
 //out_data : Output image data
 //cols, rows, :Image dimensions
 //threshold : Threshold value
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_thr_le2thr_8_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	/* Request contiguous heap memory allocation for the extended input structure */
	pAlloc = (XDAS_Int8 *) Memory_alloc(InArg_Buf_size, &wrapperMemParams);
	CInArgs = (IC6Accel_InArgs *) pAlloc;
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_thr_le2thr_8_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->indata_InArrID1 = INBUF0;
	fp0->outdata_OutArrID1 = OUTBUF0;
        ...
        const IplConvKernel *restrict mask, int cols)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_erode_bin_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	/* Request contiguous heap memory allocation for the extended input structure */
	pAlloc = (XDAS_Int8 *) Memory_alloc(InArg_Buf_size, &wrapperMemParams);
	CInArgs = (IC6Accel_InArgs *) pAlloc;
	...
	CACHE_WB_INV_INPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(mask->values,1,mask->nRows * mask->nCols);
	...
	/* Set function Id and parameter pointers for first function call */
	CInArgs->fxn[0].FxnID = IMG_ERODE_BIN_FXN_ID;
	CInArgs->fxn[0].Param_ptr_offset = sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns) + sizeof(Fxn_struct);


	/* Initialize pointers to function parameters */
	fp0 = (IMG_erode_bin_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->InArrID1 = INBUF0;
	fp0->InArrID2 = INBUF1;
        ...
        const IplConvKernel *restrict mask, int cols)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_dilate_bin_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	/* Request contiguous heap memory allocation for the extended input structure */
	pAlloc = (XDAS_Int8 *) Memory_alloc(InArg_Buf_size, &wrapperMemParams);
	CInArgs = (IC6Accel_InArgs *) pAlloc;
	...
	/* Set function Id and parameter pointers for first function call */
	CInArgs->fxn[0].FxnID = IMG_DILATE_BIN_FXN_ID;
	CInArgs->fxn[0].Param_ptr_offset = sizeof(CInArgs->size)
	        + sizeof(CInArgs->Num_fxns) + sizeof(Fxn_struct);


	/* Initialize pointers to function parameters */
	fp0 = (IMG_dilate_bin_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->InArrID1 = INBUF0;
	fp0->InArrID2 = INBUF1;
...
///* Canny
// * Author : Anol Paisal
// * Gaussian filter and Sobel filter
// */
//int C6accel_IMG_canny(C6accel_Handle hC6accel, const unsigned char * in_dat,
...
//	        + sizeof(IMG_conv_5x5_i8_c8s_Params)
//	        + sizeof(IMG_sobel_3x3_8_Params) + sizeof(CInArgs->size)
//	        + sizeof(CInArgs->Num_fxns);
//	/* Request contiguous heap memory allocation for the extended input structure */
//	pAlloc = (XDAS_Int8 *) Memory_alloc(InArg_Buf_size, &wrapperMemParams);
//	CInArgs = (IC6Accel_InArgs *) pAlloc;
...
//	/* Fill in input/output buffer descriptor parameters and manage ARM cache*/
//	/* See wrapper_c6accel_i.h for more details of operation                 */
//	CACHE_WB_INV_INPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(in_dat,0,rows * cols * sizeof(unsigned char));
//	CACHE_WB_INV_INPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(mask,1,5 * 5 *sizeof(unsigned char));
//	CACHE_WB_INV_INPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(out_dat,2,rows * cols *sizeof(unsigned char));
//	CACHE_INV_OUTPUT_BUFFERS_AND_SETUP_FOR_C6ACCEL(out_dat,0,rows * cols * sizeof(unsigned char));
//
//	/* Initialize the extended InArgs structure */
//	CInArgs->Num_fxns = 2;
//	CInArgs->size = InArg_Buf_size;
...
//	CInArgs->fxn[0].Param_ptr_offset = sizeof(CInArgs->size)
//	        + sizeof(CInArgs->Num_fxns) + MAX_FXN_CALLS * sizeof(Fxn_struct);
//	CInArgs->fxn[1].FxnID = IMG_SOBEL_3X3_8_FXN_ID;
...
//	fp0 = (IMG_conv_5x5_i8_c8s_Params *) ((XDAS_Int8*) CInArgs
//	        + CInArgs->fxn[0].Param_ptr_offset);
//	fp1 = (IMG_sobel_3x3_8_Params *) ((XDAS_Int8*) CInArgs
        ...
        float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(addsp_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (addsp_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(subsp_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/*Initialize pointers to function parameters */
	fp0 = (subsp_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(mpysp_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (mpysp_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(divsp_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (divsp_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(recipsp_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (recipsp_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(sqrtsp_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (sqrtsp_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(intsp_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (intsp_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(uintsp_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (uintsp_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(spint_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (spint_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(spuint_i_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (spuint_i_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(FtoIQN_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (FtoIQN_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQN_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQN_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt1, unsigned int qfmt2, int *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQXtoIQY_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQXtoIQY_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNtoF_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNtoF_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNint_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNint_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNfrac_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNfrac_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNabs_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNabs_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNlog_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNlog_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNsqrt_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNsqrt_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNisqrt_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNisqrt_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNexp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	//Initialize pointers to function parameters
	fp0 = (IQNexp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNpow_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	//Initialize pointers to function parameters
	fp0 = (IQNpow_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNmag_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/*Initialize pointers to function parameters*/
	fp0 = (IQNmag_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNmpy_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNmpy_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNmpyI32int_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNmpyI32int_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNmpyI32frac_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	//Initialize pointers to function parameters
	fp0 = (IQNmpyI32frac_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNmpyIQX_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNmpyIQX_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNrmpy_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNrmpy_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNrsmpy_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNrsmpy_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNdiv_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNdiv_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNsin_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNsin_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNasin_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNasin_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNcos_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/*Initialize pointers to function parameters */
	fp0 = (IQNcos_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNacos_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNacos_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNcosPU_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNcosPU_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
 ...
 */
int C6accel_MATH_IQNsinPU(C6accel_Handle hC6accel, int *ptr_x, int *ptr_r,
        unsigned int qfmt, int npoints)
{
	XDM1_BufDesc inBufDesc;
	XDM1_BufDesc outBufDesc;
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNsinPU_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNsinPU_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNatan2_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNatan2_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int *ptr_r, unsigned int qfmt, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IQNatan2PU_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (IQNatan2PU_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(sqrtsp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (sqrtsp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(sqrtdp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (sqrtdp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(rsqrtsp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (rsqrtsp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(rsqrtdp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (rsqrtdp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(logsp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (logsp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(logdp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (logdp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(expsp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (expsp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(expdp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (expdp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(exp2sp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (exp2sp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(exp2dp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (exp2dp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(exp10sp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (exp10sp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(exp10dp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (exp10dp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(sinsp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (sinsp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(sindp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (sindp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(cossp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (cossp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(cosdp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (cosdp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(atansp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (atansp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(atandp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (atandp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(recipsp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (recipsp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        int npoints)
{
	...
	// Allocate the InArgs structure as it varies in size
	// (Needs to be changed everytime we make a API call)
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(recipdp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	// Initialize pointers to function parameters
	fp0 = (recipdp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(atan2sp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (atan2sp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        double *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(atan2dp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (atan2dp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(powsp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (powsp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        double *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(powdp_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (powdp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(tw_gen_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Set function Id and parameter pointers for first function call */
	fp0 = (tw_gen_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->n = n;
...
float *ptr_f, int n) /* Number of elements */

{
	...
	/* Allocate the InArgs structure as it varies in size
	 ...
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(f2d_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Set function Id and parameter pointers for first function call */
	fp0 = (f2d_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
	/* Fill in the fields in the parameter structure */
	fp0->n = n;
...
Int C6accel_DSPF_complextorealnimg(C6accel_Handle hC6accel, float *ptr_x,
        float *ptr_y, float *ptr_r, int npoints)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_complxtorealnimg_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
...
Int C6accel_DSPF_sp_fftSPxSP(C6accel_Handle hC6accel, int npoints,
        ...
        unsigned char * ptr_brev, int n_min, int offset, int n_max)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_fftSPxSP_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_fftSPxSP_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
Int C6accel_DSPF_sp_ifftSPxSP(C6accel_Handle hC6accel, int npoints,
        float * restrict ptr_x, /* transformed data reversed *//* number of points      */
        float * restrict ptr_w, /* Input twiddle factors */
        float * restrict ptr_y, /* linear transformed data */
        unsigned char * ptr_brev, int n_min, int offset, int n_max)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_ifftSPxSP_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_ifftSPxSP_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
int nr) /* Number of lags */
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_autocor_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_autocor_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...

{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_dotprod_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Set function Id and parameter pointers for first function call */
	fp0 = (DSPF_sp_dotprod_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_mat_mul_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_mat_mul_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float *restrict ptr_y) /* Output matrix */
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_mat_mul_cplx_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_mat_mul_cplx_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_mat_trans_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_mat_trans_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_vecmul_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_vecmul_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_vecrecip_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_vecrecip_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_vecsum_sq_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_vecsum_sq_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_w_vec_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_w_vec_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float * restrict ptr_h, float * restrict ptr_y, int nh, int ny)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_fir_gen_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_fir_gen_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float * restrict ptr_h, float * restrict ptr_y, int nh, int ny)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_fir_cplx_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_fir_cplx_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float * restrict ptr_hb, float * restrict ptr_ha, int n)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_iir_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_iir_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        const short ny)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_convol_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_convol_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        float * restrict ptr_delay, float * restrict ptr_y, const int n)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(DSPF_sp_biquad_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Initialize pointers to function parameters */
	fp0 = (DSPF_sp_biquad_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int dst_cbcr_pitch)
{
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_ycbcr422sp_to_ycbcr420pl_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int dst_ycbcr_pitch)
{
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_ycbcr422sp_to_ycbcr422ile_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int src_cbcr_pitch)
{
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_ycbcr422pl_to_ycbcr422sp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        unsigned int src_cbcr_pitch)
{
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_ycbcr420pl_to_ycbcr422sp_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
        ...
        IplImage * restrict dst, unsigned int count)
{
	...
	/* Allocate the InArgs structure as it varies in size
	 (Needs to be changed everytime we make a API call)*/
	InArg_Buf_size = sizeof(Fxn_struct) + sizeof(IMG_RGB_To_Y_Params)
	        + sizeof(CInArgs->size) + sizeof(CInArgs->Num_fxns);
	...
	/* Set function Id and parameter pointers for first function call */
	//	CInArgs->fxn[0].FxnID = IMG_RGB_Y_FXN_ID; /*has bug*/
	...
	/* Initialize pointers to function parameters */
	fp0 = (IMG_RGB_To_Y_Params *) ((XDAS_Int8*) CInArgs
	        + CInArgs->fxn[0].Param_ptr_offset);
...
Int C6accel_waitAsyncCall(C6accel_Handle hC6accel) {
	...
	if (c6accelAsyncParams.asyncCallCount == 1) {
		...
		Status = UNIVERSAL_processWait(hC6accel->hUni,
		        &(c6accelAsyncParams.inBufs), &(c6accelAsyncParams.outBufs),
		        NULL, &(c6accelAsyncParams.inArgs),
		        &(c6accelAsyncParams.outArgs), UNIVERSAL_FOREVER);


		/* Free the InArgs structure */
		Memory_free(c6accelAsyncParams.pBuf, c6accelAsyncParams.pBufSize,
		        &wrapperMemParams);
		return Status;
	} else
